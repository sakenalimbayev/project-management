...
Environment variables required (see `.env`, not committed): `DATABASE_URL`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `AUTH_SECRET`, `NEXTAUTH_URL`, `BLOB_READ_WRITE_TOKEN` (Vercel Blob, for project document uploads).
...
**File storage** (`lib/project-document-storage.ts`, `prisma/schema.prisma`'s `ProjectDocument` model): Project supporting documents are the only binary-file feature in the app and are stored in Vercel Blob (`@vercel/blob`'s `put()`, `access: "public"`), keyed `projects/${projectId}/${uuid}-${fileName}`; only the resulting public URL is persisted, on `ProjectDocument.fileUrl`. Requires `BLOB_READ_WRITE_TOKEN` in the environment. Uploads go through `POST /api/project/[id]/documents`, which reuses `canManageProjectStages` for auth and `validateProjectDocumentFile` (also in `lib/project-document-storage.ts`) to enforce a 20MB size cap and an allow-list of document/image MIME types before anything reaches Blob storage; it fires the same `notifyProjectMembers`/`recordAuditLog` pair as the other project-attribute routes (attribute `"documents"`). Because the request body is `multipart/form-data`, the client wrapper (`services/api/projects/documents.ts`'s `uploadProjectDocuments`) does a raw `fetch` rather than going through the shared `fetcher` helper, which always forces a JSON `Content-Type` header. Downloads are plain `<a href={fileUrl} download>` links against the public Blob URL — no server round-trip. UI lives in `components/project-documents-section.tsx` (details page list + download links) and `components/dialog/upload-project-documents-dialog.tsx` (post-creation uploads); `/admin` has its own inline file picker for uploading at project-creation time, since no `projectId` exists until the create call returns.
